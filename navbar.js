...
app.component('test-data',{
    props:['activePage', 'pages', 'dropdowns', 'navLinkClick'],
    template:`
    <div style="height:56px"></div>
    <nav
    :class="[\`navbar-\${theme}\`, \`bg-\${theme}\`, \'navbar\', \'fixed-top\', \'navbar-expand-sm\']"
    >
    <!-- 2.1.1 Company Icon -->
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                ...
                <ul  class="navbar-nav">
                    <li  v-for="(page, index) in pages" class="nav-item">
                        <a v-bind:href="page.tab.url"
                        class="nav-link"
                        :class="{active:index == activePage}"
                        @click.prevent="navLinkClick(index)">{{page.tab.text}}</a>
                    </li>
                </ul>
                <ul class="navbar-nav">
                    <li v-for="dropdown in dropdowns" class="nav-item dropdown">
                        <a href="#" class="nav-link dropdown-toggle" id="navbarDropDown"
                            ...
                            {{dropdown.text}}</a>
                        <ul class="dropdown-menu" v-for="page in dropdown.pages" aria-labelledby="#navbarDropDown">
                            <li><a :href="page.url" class="dropdown-item">{{page.text}}</a></li>
                        </ul>
                    </li>
                </ul>
                <ul class="navbar-nav">
